...
export const BI_HELP_BLOCKS: Record<BiHelpKey, BiHelpBlock> = {
  ...
  'efficiency.laborDist': {
    title: '人员平均人力成本分布',
    paragraphs: [
      '【取值逻辑】每人平均人力成本 = 校对侧平均成本 + 审核侧平均成本，不是「平均投入时长 × 单一单价」。',
      '【公式】平均投入成本 = (校对总时长 ÷ 校对总单量) × 校对单价 + (审核总时长 ÷ 审核总单量) × 审核单价。',
      '校对/审核「总时长」为该角色埋点时长之和（分钟）；「总单量」为该校对或审核下出现过的 entrusted_info_id 去重个数。演示页用「全局成本参数」中的客服/操作单价与四分项折算；分母为 0 时不参与该项。'
    ]
  },
  'detail.conversion': {
    title: '业务明细表',
